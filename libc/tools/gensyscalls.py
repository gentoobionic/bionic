...
class State:
    ...
    def gen_linux_syscalls_h(self,odir):
        path = odir + "libc/include/sys/linux-syscalls.h"
        D( "generating "+path )
        fp = create_file( path )
...
#        fp2 = open( syscalls_stamp, "w")
    ...
    def gen_syscall_stubs(self, arch, top_builddir, syscalls_subdir):
        for sc in self.syscalls:
            ...
            if sc.has_key("asm-x86") and 'x86' == arch:
                fname = "%s%s/%s.S" % (top_builddir,syscalls_subdir,sc["func"])
                D2( ">>> generating "+fname )
                fp = create_file( fname )
                fp.write(sc["asm-x86"])
                fp.close()
    ...
    def  regenerate(self, arch, top_builddir, syscalls_subdir):
        ...
        D( "scanning " + top_builddir + syscalls_path )
        files = glob.glob( top_builddir + syscalls_path + "/*.S" )
        for f in files:
